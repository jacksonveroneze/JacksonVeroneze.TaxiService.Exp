...
export function createRider(baseUrl, headers, idUser) {
    ...
    const response = http.post(`${baseUrl}/api/v1/rides`, body, headers);
...
export function getById(baseUrl, headers, idRide) {
    const response = http.get(`${baseUrl}/api/v1/rides/${idRide}`, headers);
...
export function accept(baseUrl, headers, rideId, driveId) {
    ...
    const response = http.put(`${baseUrl}/api/v1/rides/${rideId}/accept`, bodyAcceptRide, headers);
    checker(response, 'Ride', 'Accepted', 204)
}

export function start(baseUrl, headers, rideId) {
    const response = http.put(`${baseUrl}/api/v1/rides/${rideId}/start`, {}, headers);
    checker(response, 'Ride', 'Started', 204)
}

export function finish(baseUrl, headers, rideId) {
    const response = http.put(`${baseUrl}/api/v1/rides/${rideId}/finish`, {}, headers);
    checker(response, 'Ride', 'Finished', 204)
}

export function cancel(baseUrl, headers, rideId) {
    const response = http.put(`${baseUrl}/api/v1/rides/${rideId}/cancel`, {}, headers);
